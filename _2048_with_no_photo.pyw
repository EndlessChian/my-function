...
class Block(turtle.Turtle):
    ...
    def judge(self, i,u,y,x):
        ...
        if a == 0:return False
            ...
            score += a
            bc_score.show_block(score)
            if score > top_score:
                top_score = score
                bc_top_score.show_block(top_score)
...
block2 = [[0]*4 for i in range(4)]
draw = Draw()
bc_pit = Background(('gold', space-200, 210, '步数：', (-115, 210)))
bc_time = Background(('mistyrose', space-200, 125, '用时：', (-115, 135)))
